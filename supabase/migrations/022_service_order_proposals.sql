-- GRX Management — Link público de proposta, aceite e follow-up
-- Migration: 022_service_order_proposals.sql
...
COMMENT ON COLUMN public.service_orders.proposal_token IS 'Token público para link de aceite da proposta (sem login).';
COMMENT ON COLUMN public.service_orders.proposal_sent_at IS 'Data/hora em que a proposta foi enviada ao cliente.';
COMMENT ON COLUMN public.service_orders.proposal_response IS 'Resposta do cliente: pending, accepted ou rejected.';

-- ---------------------------------------------------------------------------
-- Helpers internos (autenticados)
-- ---------------------------------------------------------------------------

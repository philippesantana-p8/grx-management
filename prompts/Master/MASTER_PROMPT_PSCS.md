...
| Parâmetros | **Senha Máster - Concessão de Acessos** (frase de recuperação + permissões por tela/sócio). **Análise** = só ver; **Alteração** = criar/editar/finalizar; **Exclusão** = apagar — enforçado em CrudPage, DRE, pátio, cadastros e OS. **Histórico de exclusões** (auditoria) — admin; SQL `apply-048-deletion-audit.sql`. |

...
| Shell / tabelas | AppShell em altura de viewport: **menu lateral fixo**. Listagens longas usam `DataTableScroll` / `.data-table-scroll`: scroll próprio + **thead sticky** (+ coluna 1ª/Ações quando fizer sentido) — Cadastros/CRUD, DRE, pátio, docs, parâmetros, etc. |
...
| OS importada c/ motorista | Import/legado com `driver_id` sem designação WhatsApp conta como **Motorista confirmado** (libera Concluir frete / acompanhamento / DRE). SQL `apply-060-import-driver-confirmed.sql` (backfill + `complete_service_order`). Sem valor motorista/ajudante na designação: **autorizado lançamento manual** em DRE → Lançamentos da empresa (conta Motorista/Ajudante); **nº da OS obrigatório** (`service_order_id`) para o **rateio por sócios**. |

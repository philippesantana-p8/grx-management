...
| OS importada c/ motorista | Import/legado com `driver_id` sem designação WhatsApp conta como **Motorista confirmado** (libera Concluir frete / acompanhamento / DRE). SQL `apply-060-import-driver-confirmed.sql` (backfill + `complete_service_order`). |

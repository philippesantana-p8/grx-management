...
| Documento único | **CNPJ/CPF não pode repetir** na mesma empresa em Clientes, Fornecedores, Motoristas e Sócios (comparação por dígitos). **Veículos:** placa única por empresa (UI + `UNIQUE company_id+plate`). SQL: `apply-053-unique-party-documents.sql`. |

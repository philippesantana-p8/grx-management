...
| Marca / logos | **Sistema (menu + login):** PSCS Systems 3D (`/pscs-logo-mark.png`, `BrandLogo` variant `mark`). **Header (cliente):** emblema da empresa sem fundo (`/grx-company-mark.png`) no lugar do badge EMPRESA. **Docs (voucher/proposta/e-mail):** logo GRX em placa. |

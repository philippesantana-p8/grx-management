...
export default function MotoristasPagamentosPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    setError(null);
    const { rows: data, error: fetchError } = await fetchDriverPaymentRows(supabase, companyId);
    if (fetchError) {
      setError(fetchError);
      setRows([]);
    } else {
  ...
  return (
    <Card>
      <CardHeader
        ...
        description="OS com motorista confirmado e valor informado na designação. Ao marcar pago, lança despesas DRE nas contas «Motorista» e «Ajudante»."
      />
      <CardBody>
